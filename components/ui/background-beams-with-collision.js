...
export const BackgroundBeamsWithCollision = ({ children, className }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        "h-screen w-screen fixed top-0 left-0 bg-black flex items-center justify-center overflow-hidden",
        className
      )}
    >
      {beams.map((beam, i) => (
      ...
      <div
        ...
        style={{
          boxShadow:
            "0 0 24px rgba(168, 85, 247, 0.5), 0 1px 1px rgba(255, 255, 255, 0.1), 0 0 0 1px rgba(168, 85, 247, 0.3), 0 0 4px rgba(139, 92, 246, 0.6), 0 16px 68px rgba(168, 85, 247, 0.4), 0 1px 0 rgba(255, 255, 255, 0.2) inset",
        }}
      ></div>
    </div>
  );
        ...
        <motion.div
          ...
          className={cn(
            "absolute left-0 top-20 m-auto h-14 w-px rounded-full " +
              "bg-gradient-to-t from-indigo-400 via-purple-500 to-transparent " +
    ...
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      ...
      {spans.map((span) => (
        <motion.span
          ...
          shadow-[0_0_10px_rgba(236,72,153,0.9)]"
        />
      ))}
    </div>
  );
